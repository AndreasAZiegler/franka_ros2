...
BalanceController::on_configure(const rclcpp_lifecycle::State& /*previous_state*/)
{
  ...
  {
    RCLCPP_FATAL(node_->get_logger(), "k_gains should be of size %d but is of size %d", num_joints, k_gains.size());
    return CallbackReturn::FAILURE;
  }
  if (d_gains.empty())
  {
  ...
  {
    RCLCPP_FATAL(node_->get_logger(), "d_gains should be of size %d but is of size %d", num_joints, d_gains.size());
    return CallbackReturn::FAILURE;
  }
  for (int i = 0; i < num_joints; ++i)
  {
...
controller_interface::return_type BalanceController::init(const std::string& controller_name)
{
  ...
  joint_position_subscriber_ = node_->create_subscription<std_msgs::msg::Float32>(
      "joint_position", rclcpp::SystemDefaultsQoS(),
      std::bind(&BalanceController::position_callback, this, std::placeholders::_1));
...
void BalanceController::publish_state(const JointTrajectoryPoint& desired_state,
